/*	RuleList.java
	...
	RuleList.java is free software: you can redistribute it and/or modify
...
public class RuleList extends AssocRuleMining {
	...
	private RuleNode(short[] ante, short[]cons, double laplaceaccuracy,double ruleaccuracy) {
	    antecedent      = ante;
	    consequent      = cons;
	    laplaceAccuracy = laplaceaccuracy;
	    ruleacc=ruleaccuracy;
	    }
	}
    ...
    protected void insertRuleintoRulelist(short[] antecedent,
    				short[] consequent, double laplaceAccuracy,double ruleacc) {

	// Create new node
	RuleNode newNode = new RuleNode(antecedent,consequent,
							laplaceAccuracy,ruleacc);
	...
	//--------------------------------------------------------------------------------------------------------
	    protected void insertRuleintoRulelist_rule(short[] antecedent,
    				short[] consequent, double laplaceAccuracy,double ruleacc) {
	...
	// Add new node to start
	if (ruleacc > startRulelist.ruleacc) {
	    newNode.next = startRulelist;
	    startRulelist  = newNode;
	    return;
	    }
	...
	RuleNode linkRuleNode = startRulelist.next;
	while (linkRuleNode != null) {
	    if (ruleacc > linkRuleNode.ruleacc) {
	        markerNode.next = newNode;
		newNode.next    = linkRuleNode;
		return;
		}
	...
	// Return
	double accuracy =
	         (double) (classCounter+1)/(double) (totalCounter+numClasses);
	return(accuracy);
	}

	 protected double getRuleAccuracy(short[] antecedent, short consequent,double x,double y) {
    	int totalCounter=0;	// Ntot
	int classCounter=0;	// Nc
	int negCounter=0;
	...
	double e=(negCounter+classCounter)*(x/(x+y));
	double f=(negCounter+classCounter)*(y/(x+y));
	double accuracy=0;
	double z=(classCounter)*Math.log(classCounter/e);
	double z1=(negCounter)*Math.log(negCounter/f);
	if(classCounter==0) accuracy=2*z1;
	else if(negCounter==0) accuracy=2*z;
	else
		accuracy=2*(z+z1);
	// Return
	return(accuracy);
    ...
    protected short classifyRecordBestKaverage(int kValue,
    				short[] classification, short[] itemSet,int choice) {
    ...
    private double[] getAverageAccuracies(int decrement,int choice) {
	...
	RuleNode linkRef = startRulelist;
	while (linkRef!=null) {
	    int index = linkRef.consequent[0]-decrement;
	    if(choice==1) {
	    accuracies[index] = accuracies[index]+linkRef.laplaceAccuracy;
	    }
	    else
	    {
	    	accuracies[index] = accuracies[index]+linkRef.ruleacc;
	    }
	    totals[index]++;
	    linkRef=linkRef.next;
	    }
	...
	while (linkRef!=null) {
	    ...
	    if (isSubset(linkRef.antecedent,itemSet)) {
	   	...
	   	RuleNode newNode = new RuleNode(linkRef.antecedent,
				linkRef.consequent,linkRef.laplaceAccuracy,linkRef.ruleacc);
	...
	while (linkRuleNode != null) {
	    ...
	    outputRule(linkRuleNode);
            jtxt.append(" " +
            		twoDecPlaces(linkRuleNode.laplaceAccuracy) + "%"+"    "+twoDecPlaces(linkRuleNode.ruleacc)+"\n");
	    number++;
	    linkRuleNode = linkRuleNode.next;
	    }
	}
